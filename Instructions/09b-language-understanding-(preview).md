---
lab:
  title: 언어 서비스를 사용하여 언어 이해 모델 만들기
...
2. **&#65291;리소스 만들기** 단추를 선택하고, 언어를 검색하고, 다음 설정을 사용하여 **언어 서비스** 리소스를 만듭니다.
    ...
    - **지역**: 미국 서부 2 또는 서유럽
    - **이름**: *고유 이름 입력*
    - **가격 책정 계층**: 무료(F) 또는 표준(S) 계층 중 하나를 선택합니다(무료를 사용할 수 없는 경우).
    - **책임 있는 AI 알림**: 동의
...
3. 이러한 발화를 추가한 후 **변경 내용 저장** 을 클릭하고 **스키마 정의** 페이지로 돌아갑니다.
...
5. 이러한 발화를 추가하고 저장한 후 **스키마 정의** 페이지로 돌아가서 다음 발화를 사용하여 **GetDate** 라는 또 다른 새 의도를 추가합니다.
...
1. 왼쪽 창에서 **모델 학습** 페이지를 선택한 다음, **학습 작업을** 선택합니다.
...
5. **모델 성능** 페이지를 선택한 다음 **시계 모델을 선택합니다**. 전체 및 의도별 평가 메트릭(정밀도, 재현율, F1 점수)과 학습 시 수행된 평가에서 생성된 혼동 행렬을 검토합니다(샘플 발화 수가 적기 때문에 일부 의도가 결과에 포함되지 않을 수 있음).   
...
6. 모델이 배포되면 **테스트 배포** 페이지에서 **프로덕션** 배포를 선택합니다.
...
1. Language Studio에서 **스키마 정의** 페이지로 돌아가서 **엔터티** 탭에서 **&#65291; 추가** 를 선택하여 새 엔터티를 추가합니다.

2. **엔터티 추가** 대화 상자에서 엔터티 이름 **Location**을 입력하고 **Learned**가 선택되었는지 확인합니다. 그런 다음, **엔터티 추가**를 클릭합니다.

3. **Location** 엔터티를 만든 후 **스키마 정의** 페이지로 돌아가**서 의도** 탭에서 **GetTime** 의도를 선택합니다.
...
5. **스키마 정의** 페이지로 돌아가서 **의도** 탭에서 **GetDate** 의도를 선택합니다.
...
5. **스키마 정의** 페이지로 돌아가서 **의도** 탭에서 **GetDay** 의도를 선택합니다.
...
2. **모델 성능** 페이지를 선택한 다음 **시계** 모델을 선택합니다. 전체, 엔터티별 및 의도별 평가 메트릭(정밀도, 재현율, F1 점수)과 학습 시 수행된 평가에서 생성된 혼동 행렬을 검토합니다(샘플 발화 수가 적기 때문에 일부 의도가 결과에 포함되지 않을 수 있음).   
...
4. 모델을 배포할 때 **배포 테스트** 페이지에서 **프로덕션** 배포를 선택한 다음, 다음 텍스트로 테스트합니다.
...
1. Language Studio의 **모델 배포** 페이지에서 **프로덕션** 배포를 선택합니다. 그런 다음, **예측 URL 가져오기**를 클릭합니다.
...
1. Language Studio에서 **질문 및 대화 언어 이해** 섹션까지 아래로 스크롤하고 **대화형 언어 이해를** 클릭하고 **Clock** 프로젝트를 선택합니다. **Clock**을 클릭하지 말고 원 모양 아이콘을 선택하여 Clock 프로젝트를 선택합니다.
